            ...
            builder: (context, state) {
              return state.maybeWhen(
                ...
                loaded: (attendance) {
                  ...
                  final latlonIn =
                      attendance.latlonIn ?? ''; // Default to empty string
                  final latlonOut = attendance.latlonOut ?? '';

                  double latitudeIn = 0.0, longitudeIn = 0.0;
                  double latitudeOut = 0.0, longitudeOut = 0.0;

                  if (latlonIn.isNotEmpty && latlonIn.contains(',')) {
                    final latlongInParts = latlonIn.split(',');
                    latitudeIn = double.tryParse(latlongInParts.first) ?? 0.0;
                    longitudeIn = double.tryParse(latlongInParts.last) ?? 0.0;
                  }

                  if (latlonOut.isNotEmpty && latlonOut.contains(',')) {
                    final latlongOutParts = latlonOut.split(',');
                    latitudeOut = double.tryParse(latlongOutParts.first) ?? 0.0;
                    longitudeOut = double.tryParse(latlongOutParts.last) ?? 0.0;
                  }
